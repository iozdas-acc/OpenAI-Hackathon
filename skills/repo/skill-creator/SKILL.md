...
- Prefer native Codex tools, local repo scripts, and existing workspace files over external vendor-specific CLIs when the workflow can be completed inside this environment.
- After adding or updating a repo-managed skill, use `scripts/sync-codex-skills.sh` to expose it to the local Codex install.
